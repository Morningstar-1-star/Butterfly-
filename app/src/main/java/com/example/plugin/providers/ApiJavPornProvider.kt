...
class ApiJavPornProvider(
    private val http: HttpBridge = HttpBridge()
) : ContentProviderApi {

    override val providerId: String = "apijav_porn"
    private val baseUrl = "https://porn.apijav.com/wp-json/myvideo/v1"
    private val httpBaseUrl = "http://porn.apijav.com/wp-json/myvideo/v1"

    override suspend fun home(pageToken: String?): PagedResult<PluginVideoItem> = withContext(Dispatchers.IO) {
        val page = pageToken?.toIntOrNull() ?: 1
        var url = "$baseUrl/posts?per_page=20&page=$page"
        var resp = http.get(url)
        if (resp.statusCode != 200) {
            url = "$httpBaseUrl/posts?per_page=20&page=$page"
            resp = http.get(url)
        }
    ...
    override suspend fun search(query: String, pageToken: String?): PagedResult<PluginVideoItem> = withContext(Dispatchers.IO) {
        ...
        var url = "$baseUrl/posts?search=$q&per_page=20&page=$page"
        var resp = http.get(url)
        if (resp.statusCode != 200) {
            url = "$httpBaseUrl/posts?search=$q&per_page=20&page=$page"
            resp = http.get(url)
        }
        if (resp.statusCode != 200) return@withContext PagedResult(emptyList())
    ...
    override suspend fun getVideo(idOrUrl: String): PluginVideoItem = withContext(Dispatchers.IO) {
        ...
        var resp = http.get(url)
        if (resp.statusCode != 200) {
            url = "$httpBaseUrl/posts/$id"
            resp = http.get(url)
        }
        if (resp.statusCode == 200) {
            val json = JSONObject(resp.body)
            val studio = json.optString("studio").ifEmpty { "APIJAV Porn" }
            return@withContext PluginVideoItem(
                id = json.optLong("id").toString().ifEmpty { id },
                title = json.optString("title", "APIJAV Porn Video"),
                uploaderName = studio,
        ...
        PluginVideoItem(
            id = id,
            title = "APIJAV Porn #$id",
            uploaderName = "APIJAV Porn",
            providerId = providerId
        )
    }
    ...
    override suspend fun getStreams(idOrUrl: String): PluginStreamInfo = withContext(Dispatchers.IO) {
        val id = extractId(idOrUrl)
        var title = "APIJAV Porn"
        var studio = "APIJAV Porn"
        var views = 0L
        var likes = 0L
        var embedUrl = ""
        var iframeHtml = ""

        var postResp = http.get("$baseUrl/posts/$id")
        if (postResp.statusCode != 200) {
            postResp = http.get("$httpBaseUrl/posts/$id")
        }
        if (postResp.statusCode == 200) {
        ...
        if (embedUrl.isEmpty() || embedUrl.contains("wp-json")) {
            var playerResp = http.get("$baseUrl/player/$id")
            if (playerResp.statusCode != 200) {
                playerResp = http.get("$httpBaseUrl/player/$id")
            }
            if (playerResp.statusCode == 200) {
        ...
        if (embedUrl.isEmpty()) {
            embedUrl = "https://porn.apijav.com/?mvembed=1&id=$id"
        }
        ...
        PluginStreamInfo(
            ...
            viewCount = views,
            likeCount = likes,
            description = iframeHtml.ifEmpty { "Embedded stream from APIJAV Porn ($studio)" },
            hlsUrl = null,
            videoStreams = listOf(
                PluginVideoStream(
                    url = embedUrl,
    ...
    override suspend fun getChannel(channelIdOrUrl: String): PluginChannel {
        return PluginChannel(id = channelIdOrUrl, name = "APIJAV Porn Studio")
    }

    override suspend fun getPlaylist(playlistIdOrUrl: String): PluginPlaylist {
        return PluginPlaylist(id = playlistIdOrUrl, title = "APIJAV Porn Playlist", uploaderName = "APIJAV Porn")
    }
    ...
    private fun parsePostsList(jsonStr: String): Pair<List<PluginVideoItem>, Boolean> {
        ...
        return try {
            ...
            for (i in 0 until arr.length()) {
                ...
                val studio = json.optString("studio").ifEmpty { "Porn" }
