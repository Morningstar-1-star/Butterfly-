...
class TmdbTorrentProvider(
                    ...
                    for (i in 0 until minOf(streamArr.length(), 6)) {
                        ...
                        if (url.isNotEmpty()) {
                            streams.add(
                                PluginVideoStream(
                                    url = url,
                                    qualityLabel = "$name: ${streamTitle.take(30)}",
    ...
    private fun parseTmdbList(jsonStr: String): Pair<List<PluginVideoItem>, Int> {
        ...
        for (i in 0 until results.length()) {
            ...
            val imgPath = if (poster.isNotEmpty()) "$IMAGE_BASE_URL$poster" else if (backdrop.isNotEmpty()) "$IMAGE_BASE_URL$backdrop" else null
