 ...
 * High-speed video catalog, search, and direct MP4/HLS stream extraction.
 */
object TnaFlixProvider {
    ...
    suspend fun getHome(limit: Int = 30, page: Int = 1): List<VideoItem> = withContext(Dispatchers.IO) {
        ...
        // 1. Swift parallel fetch across TnaFlix sections
        try {
            val liveItems = withTimeoutOrNull(4000L) {
                coroutineScope {
                    val pDef = async { parseHtml(if (safePage == 1) "$BASE_URL/popular-videos" else "$BASE_URL/popular-videos/$safePage", limit) }
                    val fDef = async { parseHtml(if (safePage == 1) "$BASE_URL/featured" else "$BASE_URL/featured/$safePage", limit) }
                    val lDef = async { parseHtml(if (safePage == 1) "$BASE_URL/latest-updates" else "$BASE_URL/latest-updates/$safePage", limit) }

                    val pRes = pDef.await()
                    if (pRes.isNotEmpty()) return@coroutineScope pRes
                    val fRes = fDef.await()
                    if (fRes.isNotEmpty()) return@coroutineScope fRes
                    val lRes = lDef.await()
            ...
            if (!liveItems.isNullOrEmpty()) {
                Log.i(TAG, "TNAFlix getHome page $safePage fetched ${liveItems.size} live videos")
    ...
    suspend fun search(query: String, limit: Int = 30, page: Int = 1): List<VideoItem> = withContext(Dispatchers.IO) {
        val clean = query.replace(Regex("(?i)tnaflix:"), "").trim()
        ...
        try {
            ...
            if (!liveSearch.isNullOrEmpty()) {
                Log.i(TAG, "TNAFlix search '$clean' fetched ${liveSearch.size} live videos")
    ...
    private fun parseHtml(url: String, limit: Int): List<VideoItem> {
        ...
        try {
            ...
            for (elem in items) {
                ...
                val videoId = fullUrl.substringAfter("tnaflix.com/").trim('/')
                ...
                val uploader = elem.selectFirst(".uploader, .channel, .author, .user")?.text()?.trim() ?: "TNAFlix HD"
                ...
                val item = VideoItem(
                    ...
                    providerId = PROVIDER_ID,
                    description = "TNAFlix HD Video Stream"
                )
                list.add(item)
            }
        } catch (e: Exception) {
            Log.w(TAG, "Failed to parse TNAFlix HTML for $url: ${e.message}")
        }
        return list
    }
