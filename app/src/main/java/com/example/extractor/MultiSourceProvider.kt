...
object MultiSourceProvider {
    ...
    suspend fun getHome(context: Context, providerId: String, limit: Int = 20, page: Int = 1): List<VideoItem> = withContext(Dispatchers.IO) {
        ...
        val customItems = when (pid) {
            ...
            "playvid" -> PlayvidProvider.getHome(limit, page)
            "txxx" -> TxxxProvider.getHome(limit, page)
            "crunchyroll" -> CrunchyrollProvider.getHome(limit, page)
            "sonyliv" -> SonyLivProvider.getHome(limit, page)
            ...
            "123av", "javplayer" -> JavVideoExtractor.getHome("123av", limit, page)
            "javtiful" -> JavVideoExtractor.getHome("javtiful", limit, page)
            "supjav" -> SupJavProvider.getHome(limit, page, context)
            "jav_all", "all_jav" -> JavVideoExtractor.getHome("jav_all", limit, page)
    ...
    suspend fun search(context: Context, providerId: String, query: String, limit: Int = 20, page: Int = 1): List<VideoItem> = withContext(Dispatchers.IO) {
        ...
        when (pid) {
            ...
            "playvid" -> PlayvidProvider.search(query, limit, page)
            "txxx" -> TxxxProvider.search(query, limit, page)
            "crunchyroll" -> CrunchyrollProvider.search(query, limit, page)
            "sonyliv" -> SonyLivProvider.search(query, limit, page)
            ...
            "123av", "javplayer" -> JavVideoExtractor.search("123av", query, limit, page)
            "javtiful" -> JavVideoExtractor.search("javtiful", query, limit, page)
            "supjav" -> SupJavProvider.search(query, limit, page, context)
            "jav_all", "all_jav" -> JavVideoExtractor.search("jav_all", query, limit, page)
